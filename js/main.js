const Main = {
  ...
  systemPrompts: {
    general: 'أنت مساعد ذكاء اصطناعي احترافي (AI Agent Pro). أجب بالعربية الفصحى أو العامية حسب سياق المستخدم. تحدث كأنك خبير أو عالم حسب ما يفضله المستخدم. أنت تعمل داخل منصة AI Agent Pro. إذا سألك المستخدم عن كيفية إعداد المنصة (مثل تغيير الألوان أو وضع مفتاح API)، أرشده بلغة بشرية بسيطة: يمكنه فتح القائمة السفلية أو الجانبية والضغط على "الإعدادات" لتغيير المظهر، أو اختيار "الشركات" لإضافة مفاتيح API لزيادة السرعة والحدود. إذا واجهت أخطاء بسبب النماذج المجانية، اقترح بلطف أن يستخدم الـ API الخاص به لضمان أفضل أداء مستقر.',
  ...
  renderDynamicPrompts() {
    ...
    if (!container) return;
    const prompts = [
      { text: "اكتب كود Python لترتيب قائمة بخوارزمية Quick Sort مع شرح كل خطوة", title: "مساعد برمجي", icon: "code", color: "card-blue" },
      { text: "اشرح لي مفهوم الشبكات العصبية التوليدية بطريقة بسيطة مع أمثلة", title: "شرح مفاهيم", icon: "cpu", color: "card-red" },
      { text: "ترجم النص التالي إلى الإنجليزية مع الحفاظ على المصطلحات التقنية", title: "ترجمة ذكية", icon: "languages", color: "card-gray" },
      { text: "أعطني 5 أفكار مبتكرة لتطبيق موبايل في مجال الصحة والرياضة", title: "أفكار إبداعية", icon: "lightbulb", color: "card-yellow" },
      { text: "قم بتحليل هذا الرابط واستخراج أهم النقاط منه: [رابط الموقع]", title: "تحليل الروابط", icon: "link", color: "card-purple" },
      { text: "اكتب خطة تسويقية متكاملة لإطلاق منتج جديد في الأسواق", title: "خطة تسويق", icon: "trending-up", color: "card-green" },
      { text: "قم بمراجعة هذا الكود واكتشاف الأخطاء الأمنية أو البرمجية فيه", title: "مراجعة الكود", icon: "shield-check", color: "card-cyan" },
      { text: "لخص لي كتاب 'العادات الذرية' في 5 نقاط رئيسية قابلة للتطبيق", title: "تلخيص كتب", icon: "book-open", color: "card-beige" }
    ];
    ...
    container.innerHTML = currentPrompts.map(p => `
      <div class="quick-prompt ${p.color}" onclick="Main.quickPrompt('${p.text}')">
        <div class="grid-icon-center"><i data-lucide="${p.icon}" style="width:100%;height:100%"></i></div>
        <div class="quick-prompt-title" style="position:relative; z-index:2; text-shadow:0 2px 4px rgba(0,0,0,0.6);">${p.title}</div>
      </div>
    `).join('');
  ...
  saveData() {
    if (localStorage.getItem('persist') !== 'false') {
      ...
      toSave.sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
      localStorage.setItem('ai_chats_pro', JSON.stringify(toSave.slice(0, 50))); 
    }
        ...
        <div class="chat-item ${isActive} ${isSelected}" 
          ...
          <div style="display:flex; justify-content:space-between; align-items:center; gap:8px;">
            ...
            <div class="chat-item-actions">
              <button class="icon-btn" style="width:24px;height:24px;padding:0" onclick="event.stopPropagation(); Main.renameChat('${c.id}')" title="إعادة تسمية">
                <i data-lucide="pencil" style="width:11px;height:11px"></i>
              </button>
              <button class="icon-btn text-danger" style="width:24px;height:24px;padding:0" onclick="event.stopPropagation(); Main.deleteChat('${c.id}')" title="حذف">
          ...
          <div style="display:flex; justify-content:space-between; align-items:center;">
            ...
            <span style="font-size:10px;color:var(--text3);white-space:nowrap;margin-right:8px;">${timeAgo}</span>
          </div>
        </div>
      `;
    });
