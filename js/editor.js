  ...
  runCode() {
    ...
    if (htmlTab || cssTab || jsTab) {
      ...
      if (!fullDoc.includes('<head>')) {
        fullDoc = `<html><head>${cssCode}</head><body>${fullDoc}${jsCode}</body></html>`;
      } else {
        fullDoc = fullDoc.replace('</head>', `${cssCode}</head>`);
        fullDoc = fullDoc.replace('</body>', `${jsCode}</body>`);
      }

      let overlay = document.getElementById('editor-preview-overlay');
      if (!overlay) {
        ...
        overlay.innerHTML = `
          <div class="editor-preview-toolbar">
            <button class="toolbar-btn" onclick="Editor.closePreview()"><i data-lucide="arrow-right" style="width:14px;height:14px"></i> رجوع للأكواد</button>
            <button class="toolbar-btn" onclick="Editor.closePreview(); Editor.toggleSplitScreen();" style="margin-right:auto;"><i data-lucide="x" style="width:14px;height:14px"></i> إغلاق</button>
          </div>
          <iframe id="editor-preview-iframe" class="editor-preview-iframe" sandbox="allow-scripts allow-modals"></iframe>
        `;
