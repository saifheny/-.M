const UI = {
  ...
  setTheme(name) {
    ...
    document.querySelector('meta[name="theme-color"]')?.setAttribute('content', name === 'dark' ? '#030712' : '#F8FAFC');
  ...
  renderModelsModal(filterText = '') {
    ...
    if (!query) {
      html += `
        <div class="models-grid-premium">
          <div class="quick-prompt" onclick="UI.selectModel('default-pro', 'AI Agent Pro'); UI.closeModal('model-picker-modal');" data-model="default-pro" style="grid-column: 1 / -1; height: 160px; background: linear-gradient(to bottom, rgba(139,92,246,0.6) 0%, #000000 100%);">
            <i data-lucide="zap" class="grid-icon" style="color:#A78BFA; opacity:0.9; width:24px; height:24px;"></i>
            <div style="position:absolute; top:12px; left:12px; z-index:5;">
              <div style="background:rgba(139,92,246,0.3);padding:4px 12px;border-radius:20px;font-size:10px;font-weight:800;color:#fff;border:1px solid rgba(139,92,246,0.5);">النظام الأساسي</div>
            </div>
            <div class="quick-prompt-title" style="font-size:24px;">AI Agent Pro</div>
            <div class="quick-prompt-sub" style="font-size:12px; color:rgba(255,255,255,0.8);">أذكى وأسرع تجربة بمعايير 2026</div>
          </div>
      `;
    } else {
    ...
    for (const [providerKey, providerData] of Object.entries(Main.ALL_MODELS)) {
      ...
      filteredModels.forEach(m => {
        ...
        html += `
          <div class="quick-prompt" onclick="${clickAction}" data-model="${m.id}" style="height:120px; background: linear-gradient(to bottom, ${colors.glow}40 0%, #000000 100%); padding:16px;">
            ...
            <div class="quick-prompt-title" style="text-align:center;">${m.name}</div>
            <div class="quick-prompt-sub" style="text-align:center; opacity:0.7; font-size:10px;">${m.desc}</div>
          </div>`;
      });
    }
